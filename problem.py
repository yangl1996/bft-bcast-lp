...
for d in dests:
    for n in nodes:
        prob += (pl.lpSum([flowEdgeVars[d][n][i] for i in nodes]) <= egress[n], f"EgressCapFlow{d}Node{n}")

# ingress caps
for d in dests:
    for n in nodes:
        prob += (pl.lpSum([flowEdgeVars[d][i][n] for i in nodes]) <= ingress[n], f"IngressCapFlow{d}Node{n}")
